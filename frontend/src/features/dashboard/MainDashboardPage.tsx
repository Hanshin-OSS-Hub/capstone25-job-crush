...
const AnalysisCard = ({ data }: { data: AnalysisData }) => {
  // 상태에 따라 뱃지 색상 다르게 적용
  const statusBadgeClass = data.status === '완료'
    ...
    <div className="rounded-lg border border-stroke bg-white p-6 shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          <div className={`flex h-14 w-14 items-center justify-center rounded-lg ${data.logoBgColor} ${data.logoTextColor} dark:bg-meta-4 dark:text-white`}>
            <FaBuilding className="h-6 w-6" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-black dark:text-white">{data.company}</h3>
            <p className="text-sm font-medium text-body">{data.role}</p>
          </div>
        </div>
        <span className={`inline-flex rounded-full bg-opacity-10 px-3 py-1 text-sm font-medium ${statusBadgeClass}`}>
          {data.status}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 mb-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-black dark:text-white">{data.fitScore}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-body">직무 적합도</span>
          <span className="text-sm font-bold text-black dark:text-white">{data.jobScore}</span>
        </div>
      </div>
    ...
    <div className="flex flex-col gap-8 p-6 md:p-8 2xl:p-10">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-stroke bg-white dark:bg-boxdark py-6 px-7.5 shadow-default">
          <h4 className="text-title-md font-semibold text-black dark:text-white">총 분석 횟수</h4>
        ...
        {/* 평균 점수 */}
        <div className="rounded-lg border border-stroke bg-white dark:bg-boxdark py-6 px-7.5 shadow-default">
          <h4 className="text-title-md font-semibold text-black dark:text-white">평균 점수</h4>
          <div className="mt-4 flex items-end justify-between">
        ...
        {/* 면접 연습 */}
        <div className="rounded-lg border border-stroke bg-white dark:bg-boxdark py-6 px-7.5 shadow-default">
          <h4 className="text-title-md font-semibold text-black dark:text-white">면접 연습</h4>
          <div className="mt-4 flex items-end justify-between">
            <p className="text-3xl font-bold text-black dark:text-white">
        ...
        {/* 이번 달 분석 */}
        <div className="rounded-lg border border-stroke bg-white dark:bg-boxdark py-6 px-7.5 shadow-default">
          <h4 className="text-title-md font-semibold text-black dark:text-white">이번 달 분석</h4>
          <div className="mt-4 flex items-end justify-between">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-black dark:text-white">최근 분석 기록</h2>
          <button className="text-sm font-medium text-primary hover:underline">
            전체보기
          </button>
        </div>
