...
const ResumeAnalysisPage = () => {
  ...
  const handleAnalyzeClick = () => {
    // 유효성 검사 (간단 예시)
    // if (!formData.companyName) return alert('기업명을 입력해주세요.');
    ...
    setTimeout(() => {
      ...
      alert("분석이 완료되었습니다! (결과 페이지로 이동)");
      // navigate('/analysis/result'); // 나중에 결과 페이지가 생기면 이걸로 이동
    }, 3000);
  };
  ...
  {/* 3. 상태에 따라 아이콘과 텍스트 변경 */}
        ...
        <button
        onClick={() => navigate(-1)} // -1은 이전 페이지로 이동
